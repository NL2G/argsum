...
cached_quality_scorer = torch.load(
    "models/quality_scorer/bert_ft_topic_np_mace-p/2024-May-29_15-58-03/best_model.pt",
    map_location=device,
)


# BarH
def get_barh_candidates(
    arguments,
    topic,
    quality_scorer=None,
    quality_scorer_t=0.7,
    min_proportion_candidates=0.1,
):
    """
    ...
    aqs = np.array(
        get_quality_scores(model=quality_scorer, arguments=arguments, topic=topic)
    )
    ...
    if len(candidates) < min_n_candidates:
        ...
        candidates_without_quality_score_aqs = aqs[
            [cand[0] for cand in candidates_without_quality_score]
        ]
        sorted_candidates_without_quality_score = [
            cand
            for _, cand in sorted(
                zip(
                    candidates_without_quality_score_aqs,
                    candidates_without_quality_score,
                ),
                reverse=True,
            )
        ...
        if len(sorted_candidates_without_quality_score) > diff_n_candidates:
            candidates += [
                sorted_candidates_without_quality_score[j]
                for j in [i for i in range(diff_n_candidates)]
            ]
        else:
            candidates += sorted_candidates_without_quality_score
            diff_n_candidates = int(min_n_candidates - len(candidates))
            remaining_arguments_aqs = aqs[[arg[0] for arg in remaining_arguments]]
            sorted_remaining_arguments = [
                arg
                for _, arg in sorted(
                    zip(remaining_arguments_aqs, remaining_arguments), reverse=True
                )
            ]
            candidates += [
                sorted_remaining_arguments[j]
                for j in [i for i in range(diff_n_candidates)]
            ]
    return [cand[1] for cand in candidates]


def get_barh_key_points(
    arguments,
    candidates,
    match_scorer=None,
    match_scorer_t=0.856,
    scorer_cands=None,
    scorer_cands_t=None,
):
    """
    ...
    if match_scorer == None:
        ...
        match_scorer = torch.load(
            "models/match_scorer/bi_encoder/roberta_tp/2024-Feb-20_16-23-49",
            map_location=device,
        )
    ...
    matched_cands_args = get_best_match_plus_threshold_dict(
        match_scorer=match_scorer,
        arguments=arguments,
        candidates=candidates,
        match_scorer_t=match_scorer_t,
    )
    unmatched_cands = [
        cand for cand in candidates if cand not in list(matched_cands_args.keys())
    ]
    # Sort matched candidates with corresponding matched arguments according to number of matches
    matched_cands_args_sorted = {}
    for k in sorted(
        matched_cands_args, key=lambda k: len(matched_cands_args[k]), reverse=True
    ):
    ...
    if len(matched_cands_sorted) > 0:
        ms_matched_cands_sorted = get_match_score_matrix(
            scorer_cands, matched_cands_sorted, matched_cands_sorted
        )
        ...
        for i, cand_1 in enumerate(matched_cands_sorted):
            for j, cand_2 in enumerate(matched_cands_sorted):
                if i < j:
                    ms = (
                        ms_matched_cands_sorted[i, j] + ms_matched_cands_sorted[i, j]
                    ) / 2
                    if ms > scorer_cands_t:
                        ...
                        if cand_2 in matched_cands_args.keys():
                            matched_cands_to_remove = np.append(
                                matched_cands_to_remove, cand_2
                            )
                            matched_args_to_remove = np.append(
                                matched_args_to_remove, matched_cands_args[cand_2]
                            )
        ...
        rem_args = np.concatenate(
            [matched_cands_to_remove, matched_args_to_remove, unmatched_cands]
        )
        # Get matched key points with corresponding matched arguments for remaining arguments
        if len(rem_args) > 0:
            matched_kps_rem_args = get_best_match_plus_threshold_dict(
                match_scorer=match_scorer,
                arguments=rem_args,
            ...
            for kp in matched_kps_rem_args.keys():
                matched_cands_args[kp] = np.append(
                    matched_cands_args[kp], matched_kps_rem_args[kp]
                )
...
def get_barh_classification_sums(
    arguments,
    topic,
    stance=None,
    quality_scorer=None,
    ...
    if use_llm == False:
        # Get candidates
        candidates = get_barh_candidates(
            arguments=arguments,
            topic=topic,
        ...
        # Extract key points
        key_points = get_barh_key_points(
            arguments=arguments,
            candidates=candidates,
            match_scorer=match_scorer,
            match_scorer_t=match_scorer_t,
            scorer_cands=scorer_cands,
            scorer_cands_t=scorer_cands_t,
        )
    elif use_llm == "candidates":
        # Get candidates
        candidates = get_llm_classification_sums(
            arguments,
            topic,
            stance,
            purpose="candidates",
        ...
        # Generate key points
        key_points = get_llm_classification_sums(
            arguments,
            topic,
            stance,
            purpose="key_points",
    ...
    if key_points != None:
        ...
        matches = get_best_match_plus_threshold(
            arguments=arguments,
            key_points=list(key_points.values()),
            match_scorer=match_scorer,
            match_scorer_t=final_match_scorer_t,
        )
...
# SMatchToPr
def get_smatchtopr_candidates(
    arguments, topic, quality_scorer_t=0.8, min_proportion_candidates=0.1
):
    """
    Extracts candidates from arguments given the topic.
    ...
    if len(candidates) < min_n_candidates:
        ...
        if len(sorted_candidates_without_quality_score) > diff_n_candidates:
            candidates += [
                sorted_candidates_without_quality_score[j][1]
                for j in [i for i in range(diff_n_candidates)]
            ]
            candidate_qualities += [
                sorted_candidates_without_quality_score[j][0]
                for j in [i for i in range(diff_n_candidates)]
            ]
        else:
            ...
            sorted_remaining_arguments = [
                (aqs, arg)
                for aqs, arg in sorted(
                    zip(remaining_arguments_aqs, remaining_arguments), reverse=True
                )
            ]

            candidates += [
                sorted_remaining_arguments[j][1]
                for j in [i for i in range(diff_n_candidates)]
            ]
            candidate_qualities += [
                sorted_remaining_arguments[j][0]
                for j in [i for i in range(diff_n_candidates)]
            ]
...
def get_smatchtopr_key_points(
    ...
    if match_scorer == None:
        match_scorer = SentenceTransformer(
            "models/match_scorer/bi_encoder/roberta_tp/2024-Feb-20_16-23-49"
        )
    if scorer_cands == None:
        scorer_cands = match_scorer
    # Compute match scores of candidates
    match_score_matrix = get_match_score_matrix(
        model=match_scorer, arguments=candidates, candidates=candidates, topic=topic
    )
    match_score_matrix[match_score_matrix < match_scorer_pr_t] = 0
    # Apply pagerank and get ordered candidates
    pr_importance_scores = pagerank(
        match_score_matrix, personalize=np.array(candidate_qualities), p=damping_factor
    )
    ranked_candidates = sorted(
        list(zip(candidates, pr_importance_scores)), key=lambda x: -x[1]
    )
    # Filter candidates and get key points
    kps = [ranked_candidates[0][0]]
    for cand in ranked_candidates[1:]:
        match_scores = get_match_scores(
            model=scorer_cands,
            arguments=[cand[0] for i in range(len(kps))],
            candidates=kps,
            topic=topic,
        )
...
def get_smatchtopr_classification_sums(
    arguments,
    topic,
    stance,
    match_scorer=None,
    ...
    if use_llm == False:
        # Get candidates
        candidates, candidate_qualities = get_smatchtopr_candidates(
            arguments=arguments,
            topic=topic,
            quality_scorer_t=quality_scorer_t,
            min_proportion_candidates=min_proportion_candidates,
        ...
        # Extract key points
        key_points = get_smatchtopr_key_points(
            candidates=candidates,
            topic=topic,
            candidate_qualities=candidate_qualities,
            match_scorer=match_scorer,
            match_scorer_pr_t=match_scorer_pr_t,
        ...
        candidate_qualities = get_quality_scores(
            model=cached_quality_scorer,
...
def get_llm_classification_sums(
    arguments,
    topic,
    stance,
    purpose="candidates",
    ...
    if sum_max_num != None:
        ...
        num_token = (
            number_dict[sum_min_num]
            + " to "
            + number_dict[sum_min_num + sum_min_num_plus]
        )
...
"""[
        ...
        num_token=num_token, sum_token_length=number_dict[sum_token_length]
    )
    if few_shot == True:
        system_message += """
...
"""[
            ...
            sum_token_length=number_dict[sum_token_length],
...
"""[
            ...
            sum_max_num=number_dict[sum_max_num]
        )
    # Set API client
    ...
    for topic in topics:
        for stance in stances:
            ...
            if specification == "bm":
                for comb in classification_separation_dict["bm"].keys():
                    if (
                        len(
                            list(
                                set(
                                    classification_sums_dict["bm"][comb][topic][stance][
                                        "sum_ids"
                                    ]
                                )
                            )
                        ...
                        if (
                            classification_separation_dict["bm"][comb][topic][stance][
                                "ari"
                            ]
                            > best_ari
                        ):
                            best_ari = classification_separation_dict["bm"][comb][
                                topic
                            ][stance]["ari"]
                            comp = classification_separation_dict["bm"][comb][topic][
                                stance
                            ]["comp"]
                            prop_matched = classification_separation_dict["bm"][comb][
                                topic
                            ][stance]["prop_matched"]
                            best_param = comb
                            sum = classification_sums_dict["bm"][comb][topic][stance][
                                "sums"
                            ]
                            sum_ids = classification_sums_dict["bm"][comb][topic][
                                stance
                            ]["sum_ids"]
                ...
                for comb in classification_separation_dict["bm_plus_th"].keys():
                    if (
                        len(
                            list(
                                set(
                                    [
                                        id
                                        for id in classification_sums_dict[
                                            "bm_plus_th"
                                        ][comb][topic][stance]["sum_ids"]
                                        if id != -1
                                    ]
                                )
                            )
                        ...
                        if (
                            classification_separation_dict["bm_plus_th"][comb][topic][
                                stance
                            ]["noise"]["ari"]
                            > best_ari
                        ):
                            best_ari = classification_separation_dict["bm_plus_th"][
                                comb
                            ][topic][stance]["noise"]["ari"]
                            comp = classification_separation_dict["bm_plus_th"][comb][
                                topic
                            ][stance]["noise"]["comp"]
                            prop_matched = classification_separation_dict["bm_plus_th"][
                                comb
                            ][topic][stance]["noise"]["prop_matched"]
                            best_param = comb
                            sum = classification_sums_dict["bm_plus_th"][comb][topic][
                                stance
                            ]["sums"]
                            sum_ids = classification_sums_dict["bm_plus_th"][comb][
                                topic
                            ][stance]["sum_ids"]
                ...
                for comb in classification_separation_dict["bm_plus_th"].keys():
                    if (
                        ...
                        if (
                            ...
                            best_ari = classification_separation_dict["bm_plus_th"][
                                comb
                            ][topic][stance]["no_noise"]["ari"]
                            comp = classification_separation_dict["bm_plus_th"][comb][
                                topic
                            ][stance]["no_noise"]["comp"]
                            prop_matched = classification_separation_dict["bm_plus_th"][
                                comb
                            ][topic][stance]["no_noise"]["prop_matched"]
                            best_param = comb
                            sum = classification_sums_dict["bm_plus_th"][comb][topic][
                                stance
                            ]["sums"]
            ...
            if sum != None:
                ...
                for sum_id in sum.keys():
                    if int(sum_id) in sum_ids:
                        ...
                        summaries_concat += (
                            np.array(
                                [
                                    [summary] * len(arguments_classification)
                                    for summary in summaries_classification
                                ]
                            )
                            .flatten()
                            .tolist()
                        )
                        arguments_concat += arguments_classification * len(
                            summaries_classification
                        )
                        idx_concat.append(
                            [
                                i
                                for i in range(
                                    ...
                                    len(
                                        arguments_classification
                                        * len(summaries_classification)
                                    )
                                    + 1,
                                    len(arguments_classification),
                                )
                            ]
                        )
                        scores_concat = match_evaluation_callable(
                            arguments=arguments_concat, summaries=summaries_concat
                        )
                ...
                for i, sum_id in enumerate(
                    [id for id in list(sum.keys()) if int(id) in sum_ids]
                ):
                    if int(sum_id) in sum_ids:
                        classification_idx_concat = idx_concat[i]
                        classification_scores_concat = scores_concat[
                            current_idx_concat : current_idx_concat
                            + classification_idx_concat[-1]
                        ]
                        scores_splitted = [
                            float(
                                np.round(
                                    np.mean(
                                        classification_scores_concat[
                                            classification_idx_concat[
                                                i
                                            ] : classification_idx_concat[i + 1]
                                        ]
                                    ),
                                    4,
                                )
                            )
                            for i in range(len(classification_idx_concat) - 1)
                        ]
                        if type(sum[sum_id]) == str:
                    ...
                    dict(
                        zip(
                            ...
                            [
                                "",
                                "",
                                np.round(results["best_ari"].mean(), 4),
                                np.round(results["comp"].mean(), 4),
                                np.round(results["prop_matched"].mean(), 4),
                                "",
                                "",
                                np.round(results["sum_score"].mean(), 4),
                            ],
                        )
                    )
                ]
