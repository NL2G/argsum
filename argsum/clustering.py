...
from argsum._utils_ import (
    set_summetix_api_login,
    get_summetix_api_login,
    get_summetix_api_key_id,
)
...
def get_summetix_clusters(arguments, threshold=0.675, min_cluster_size=4, topic=None):
    ...
    response = requests.post(
        api_url, data=json.dumps(payload), headers={"Content-Type": "application/json"}
    ).json()
    ...
    unclustered_argument_ids = [
        int(arg_with_id[0])
        for arg_with_id in arguments_with_ids
        if int(arg_with_id[0]) not in clustered_argument_ids
    ]
    # Get sorted (argument_id, cluster_id) pairs of clustered and unclustered arguments
    sorted_argument_ids_cluster_ids = sorted(
        list(zip(clustered_argument_ids, cluster_ids))
        + list(
            zip(
                unclustered_argument_ids,
                [-1 for i in range(len(unclustered_argument_ids))],
            )
        )
    )
...
# USKPM
def get_bertopic_clusters(
    arguments, n_neighbors=3, n_components=2, min_dist=0.0, min_cluster_size=4, seed=753
):
    """
...
# USKPM
def get_uskpm_clusters(
    arguments, n_neighbors=3, n_components=2, min_dist=0.0, min_cluster_size=4, seed=473
):
    """
    Performs soft argument clustering utilizing BERTopic.
    n_neighbors:        Number of neighboring samples used when making the manifold approximation
...
def get_iterative_clusters(arguments, cluster_ids, threshold, sentence_model=None):
    ...
    clustered_arguments = [
        arguments_with_ids[i]
        for i in range(len(arguments_with_ids))
        if cluster_ids[i] != -1
    ]
    clustered_arguments_cluster_ids = [
    ...
    current_clusters = dict(
        zip(
            list(set(clustered_arguments_cluster_ids)),
            [[] for i in range(len(list(set(clustered_arguments_cluster_ids))))],
        )
    )
    if len(current_clusters) > 0:
        ...
        for i, arg in enumerate(clustered_arguments):
            current_clusters[clustered_arguments_cluster_ids[i]].append(
                clustered_arguments[i]
            )
        # Get unclustered arguments
        unclustered_arguments = [
            arguments_with_ids[i]
            for i in range(len(arguments_with_ids))
            if cluster_ids[i] == -1
        ]
        ...
        for arg in unclustered_arguments:
            ...
            for clu in list(current_clusters.keys()):
                clu_embedding = np.mean(
                    sentence_model.encode([arg[1] for arg in current_clusters[clu]]),
                    axis=0,
                )
                sims.append(
                    cosine_similarity(
                        arg_embedding.reshape(1, -1), clu_embedding.reshape(1, -1)
                    )
                )
        ...
        reversed_current_clusters = dict(
            (v1[0], k) for k, v in current_clusters.items() for v1 in v
        )
        sorted_reversed_current_clusters = dict(
            sorted(reversed_current_clusters.items())
        )
        # Return cluster_ids
        try:
...
def get_match_clusters(
    match_score_matrix, threshold=0.675, min_cluster_size=4, method=1
):
    """
    ...
    labels_dict = dict(
        zip(
            sorted(list(set(cluster_ids))),
            [i for i in range(-1, len(list(set(cluster_ids))) - 1)],
        )
    )
...
def get_summetix_cluster_sums(arguments, cluster_ids, topic=None, stance=None):
    ...
    argument_ids_clusters = [
        [ids_args[i][0] for i in range(len(ids_args)) if cluster_ids[i] == cluster_id]
    ...
    for arg_ids_clu in argument_ids_clusters:
        # Set payload
        payload_get_aspects = {
            "arguments": dict([arg for arg in ids_args if arg[0] in arg_ids_clu]),
            "apiKeyId": api_id,
            "apiKey": api_key,
        }
        # Get response
        response_get_aspects = requests.post(
            api_url_get_aspects,
            data=json.dumps(payload_get_aspects),
    ...
    # Set payload
    payload_get_aspect_topics = {
        "clusters": dict(zip(unique_cluster_ids, argument_ids_clusters)),
    ...
    # Get response
    response_get_aspect_topics = requests.post(
        api_url_get_aspect_topics,
        data=json.dumps(payload_get_aspect_topics),
        headers={"Content-Type": "application/json"},
    ).json()
    # Return summaries
    return dict(
        sorted(
            zip(
                [
                    int(id)
                    for id in list(response_get_aspect_topics["clusterLabelMap"].keys())
                ],
                [
                    [sum]
                    for sum in list(
                        response_get_aspect_topics["clusterLabelMap"].values()
                    )
                ],
            )
        )
...
def preprocess_input_t5(df):
    ...
    quality_scorer = torch.load(
        "models/quality_scorer/bert_ft_topic_np_mace-p/2024-Feb-24_13-34-37/best_model.pt"
    )
    ...
    for summary in summaries_unique:
        ...
        sorted_args_summary = [
            arg for aq, arg in sorted(zip(aqs, args_summary), reverse=True)
        ]
        text = (
            "summarize: "
            + stance_dict[stance_summary]
            + " "
            + topic_summary
            + " "
            + " ".join(sorted_args_summary)
        )
        # text = 'summarize: ' + ' '.join(sorted_args_summary)
        preprocessed_df = pd.concat(
            [preprocessed_df, pd.DataFrame({"text": text, "label": [summary]})],
            ignore_index=True,
        )
...
def train_t5(
    ...
    tokenized_text_train = tokenizer(
        df_train["text"].to_list(),
        max_length=max_length_text,
        truncation=True,
        padding="max_length",
    )
    tokenized_text_dev = tokenizer(
        df_dev["text"].to_list(),
        max_length=max_length_text,
        truncation=True,
        padding="max_length",
    )
    tokenized_label_train = tokenizer(
        df_train["label"].to_list(),
        max_length=max_length_label,
        truncation=True,
        padding="max_length",
    )["input_ids"]
    tokenized_label_dev = tokenizer(
        df_dev["label"].to_list(),
        max_length=max_length_label,
        truncation=True,
        padding="max_length",
    )["input_ids"]
    tokenized_ds_train = Dataset.from_dict(
        {
            "input_ids": tokenized_text_train["input_ids"],
            "attention_mask": tokenized_text_train["attention_mask"],
            "labels": tokenized_label_train,
        }
    )
    tokenized_ds_dev = Dataset.from_dict(
        {
            "input_ids": tokenized_text_dev["input_ids"],
            "attention_mask": tokenized_text_dev["attention_mask"],
            "labels": tokenized_label_dev,
        }
    )
    tokenized_ds_train.set_format(
        type="pt", columns=["input_ids", "attention_mask", "labels"]
    )
    tokenized_ds_dev.set_format(
        type="pt", columns=["input_ids", "attention_mask", "labels"]
    )
    # Set seed
    set_seed(seed)
...
def get_t5_cluster_sums(
    ...
    model = torch.load(
        "models/summary_generator/flan_t5_cluster/best_model.pt", map_location="cpu"
    )
    model.to("mps")
    ...
    for cluster_id in unique_cluster_ids:
        args = [
            arguments[i] for i in range(len(arguments)) if cluster_ids[i] == cluster_id
        ]
        ...
        text = (
            ...
            + topic
            + " "
            + " ".join(sorted_args_cluster)
        )
        # text = 'summarize: ' + ' '.join(sorted_args_cluster)
        texts.append(text)
    tokenized_texts = tokenizer(
        texts, max_length=max_length_text, truncation=True, padding="max_length"
    )
...
of the arguments contained in the cluster. A key point should not exceed a length of {sum_token_length} tokens, but it can be {sum_token_length} long if necessary 
to capture compound key points.
"""[
        1:-1
    ].format(
        num_token=num_token,
    ...
    if few_shot == True:
        system_message += """
Here are three examples of good key points: "School uniform reduces bullying" is an opposing key point on the topic "We should 
abandon the use of school uniform" and "Guns lead to accidental deaths" is a supporting key point on the topic "We should 
abolish the right to keep and bear arms". "High levels of crime and gun violence." is an opposing key point on the topic "The USA is a good country to live in".
"""[
            :-1
        ]
...
"""[
                ...
                kp_token=kp_token,
                topic=topic,
                arguments='", "'.join(clusters[cluster]),
            )
...
"""[
            ...
            kp_token=kp_token,
            topic=topic,
            num_clusters=len(list(clusters.keys())),
        )
...
"""[
                ...
                cluster_id=cluster_id, arguments='", "'.join(clusters[cluster_id])
            )
        user_message += """
Since argument clusters are not optimal, they may contain arguments that do not actually belong together. Therefore, make sure that 
    ...
    for sum in sums:
        if sum not in ["", " ", "  "]:
            ...
            for subsum in splitted_sum:
                ...
                if len(subsum) > 0 and ((subsum[0] == '"') or (subsum[0] == "'")):
                    subsum = subsum[1:]
                if len(subsum) > 0 and ((subsum[-1] == '"') or (subsum[-1] == "'")):
                    subsum = subsum[:-1]
                # Check for bullets
                if len(subsum) > 0 and (subsum[0] == "-"):
                    subsum = subsum[2:]
                try:
                    if len(subsum) > 0 and type(int(subsum[0])) == int:
                        subsum = subsum[3:]
                except:
                    pass
...
def get_llm_cluster_sums(
    ...
    system_messages, user_messages = get_llm_messages(
        clusters=clusters,
        topic=topic,
        stance=stance,
    ...
    if llm in ["gpt-4o"]:
        ...
        for i in tqdm(
            range(len(system_messages)),
            position=0,
            desc="Get GPT-4o Cluster Sums",
            leave=False,
            colour="green",
        ):
            # Create model input
            ...
            while (success == False) and (n_trails <= 2):
                try:
                    ...
                    if optimization == "local":
                        ...
                        modified_outputs[i] += (
                            np.array(modified_output).flatten().tolist()
                        )
                        if len(modified_output) == n:
                            success = True
                        else:
                            n_trails += 1
                            print("failed due to insufficient number of outputs")
                        ...
                        output = [
                            [
                                sum.strip()
                                for sum in re.split(r"[*c*C]*luster* *\d*:", output[i])
                                if sum.strip() != ""
                            ]
                        ...
                        for j in range(len(outputs)):
                            modified_outputs[j] += (
                                np.array(modify_gpt_output(outputs[j]))
                                .flatten()
                                .tolist()
                            )
                        if len(modified_outputs) == len(unique_cluster_ids):
                            for i in range(len(unique_cluster_ids)):
                                if len(modified_outputs[i]) == n:
                                    ...
                                    n_trails += 1
                                    print(
                                        f"failed due to insufficient number of outputs for cluster {i}"
                                    )
                        else:
                            n_trails += 1
                            print("failed due to mismatch in number of clusters")
    ...
    summaries = dict(
        zip(
            list(clusters.keys()),
            [out[0] if len(out) == 1 else out for out in modified_outputs],
        )
    )
    if noise == True:
