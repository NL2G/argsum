...
class quality_scorer(nn.Module):
    ...
    def forward(
        ...
        if labels is not None:
            loss_fct = nn.MSELoss()
            loss = loss_fct(
                self.sigmoid(logits.view(-1, 1)),
                labels.view(-1, 1).to(dtype=torch.float32),
            )
...
def train_quality_scorer(
    ...
    if model == None:
        if pooling == False:
            ...
            model = AutoModelForSequenceClassification.from_pretrained(
                pretrained_model_name_or_path=model_path, num_labels=1
            )
            suffix = "p"
    # Create tokenized datasets
    tokenized_ds_train = Dataset.from_pandas(df_train).map(
    ...
    tokenized_ds_dev = Dataset.from_pandas(df_dev).map(
        lambda example: tokenizer(
            example["argument"],
            example["topic"],
            max_length=128,
    ...
    tokenized_ds_train = tokenized_ds_train.add_column(
        "label", tokenized_ds_train[target_name]
    )
    tokenized_ds_dev = tokenized_ds_dev.add_column(
        "label", tokenized_ds_dev[target_name]
    )
    # Set dataset formats to pytorch
    tokenized_ds_train.set_format(
        type="pt", columns=["input_ids", "attention_mask", "token_type_ids", "label"]
    )
...
def get_quality_scores(
    model, arguments, topic, stance=None, file_name=None, path=None, sleep_time=0, n=6
):
    """
    Quality scoring pipeline
    """
    if str(type(model)) in [
        "<class 'argsum.tools.quality_scorer'>",
        ...
        # Tokenize
        tokenized_ds = Dataset.from_dict(arg_top_dict).map(
            lambda example: tokenizer(
                example["argument"],
                example["topic"],
                max_length=128,
        ...
        tokenized_ds.set_format(
            type="pt", columns=["input_ids", "attention_mask", "token_type_ids"]
        )
        # Create data loader
        ...
        api_key = os.getenv("DEBATER_API_KEY", "YOUR_DEBATER_API_KEY")
        ...
        if type(topic) == str:
            ...
            topic_ = topic
        arg_top_dicts = [
            {"sentence": arguments[i], "topic": topic_[i]}
        ...
        for i in range(0, len(arg_top_dicts), n):
            ...
            runtimes += [np.round((time() - start_time) / n, 2)] * len(
                arg_top_dicts[i : i + n]
            )
...
def eval_quality_scorer(model, df_test, output_dir=None):
    ...
    if model != "debater_api":
        ...
        scores = get_quality_scores(
            "debater_api", arguments=args, topic=tops, sleep_time=0, n=len(args)
        )
...
class match_scorer_np(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None):
        ...
        if labels is not None:
            ...
            loss = loss_fct(
                logits.view(-1, 1), labels.view(-1, 1).to(dtype=torch.float32)
            )
...
def train_cross_match_scorer(
        ...
        if include_topic == False:
            ...
            tokenized_ds_train = Dataset.from_pandas(df_train).map(
                lambda example: tokenizer(
                    example["argument"],
                    [
                        example["topic"][i] + " " + example["key_point"][i]
                        for i in range(len(example["key_point"]))
                    ],
                    max_length=128,
                    truncation=True,
                    padding="max_length",
        ...
        if include_topic == False:
            ...
            tokenized_ds_train = Dataset.from_pandas(df_train).map(
                lambda example: tokenizer(
                    example["sentence_1"],
                    [
                        example["topic"][i] + " " + example["sentence_2"][i]
                        for i in range(len(example["sentence_2"]))
                    ],
                    max_length=128,
                    truncation=True,
                    padding="max_length",
    ...
    # Set dataset formats to pytorch
    tokenized_ds_train.set_format(
        type="pt", columns=["input_ids", "attention_mask", "label"]
    )
    tokenized_ds_dev.set_format(
        type="pt", columns=["input_ids", "attention_mask", "label"]
    )
...
def get_match_scores(model, arguments, candidates, topic=None, roberta_tokenizer=None):
    """
    ...
    if str(type(model)) in [
        ...
        if topic == None:
            ...
            arg_cand_dict = {
                "argument": arguments,
                "candidate": [topic + " " + cand for cand in candidates],
            }
        # Tokenize
        tokenized_ds = Dataset.from_dict(arg_cand_dict).map(
            lambda example: tokenizer(
                example["argument"],
                example["candidate"],
                max_length=128,
        ...
        with torch.no_grad():
            for batch in tqdm(
                ...
                logits = model(**tokenized_dict).logits
                scores = np.append(
                    scores, sigmoid(logits).to("cpu").detach().numpy().flatten()
                )
        ...
        if topic != None:
            if str(type(model.tokenizer)) in [
                "<class 'transformers.models.mpnet.tokenization_mpnet_fast.RobertaTokenizerFast'>",
                "<class 'transformers.models.roberta.tokenization_roberta_fast.RobertaTokenizerFast'>",
            ]:
                sep = model.tokenizer.sep_token
                candidates = [
                    topic + " " + sep + sep + " " + cand for cand in candidates
                ]
            elif (
                str(type(model.tokenizer))
...
def get_match_score_matrix(
    ...
    if str(type(model)) in [
        ...
        if comp_strategy == "standard":
            ...
            score_matrix = get_match_scores(
                model, args, cands, topic, roberta_tokenizer=roberta_tokenizer
            ).reshape(len(arguments), len(candidates))
            if len(arguments) == len(candidates) and np.all(arguments == candidates):
            ...
            if comp_strategy == "quadratic_all":
                scores = get_match_scores(
                    model,
                    args_upper_diag + args_lower_diag,
                    cands_upper_diag + cands_lower_diag,
                    topic,
                )
                ...
                scores = get_match_scores(
                    model, args_upper_diag, cands_upper_diag, topic
                )
                score_matrix[idx_pairs_upper_diag] = scores
                score_matrix[idx_pairs_lower_diag] = score_matrix.T[
                    idx_pairs_lower_diag
                ]
            elif comp_strategy == "quadratic_lower":
                scores = get_match_scores(
                    model, args_lower_diag, cands_lower_diag, topic
                )
                score_matrix[idx_pairs_lower_diag] = scores
                score_matrix[idx_pairs_upper_diag] = score_matrix.T[
                    idx_pairs_upper_diag
                ]
                ...
                reverse_idx_tuple_pairs_lower_diag = [
                    (idx_pairs_lower_diag[1][i], idx_pairs_lower_diag[0][i])
                    for i in range(len(idx_pairs_upper_diag[0]))
                ]
                mean_scores = []
                for i in range(len(idx_pairs_upper_diag[0])):
                    upper_score = upper_scores[i]
                    upper_idx_n, upper_idx_m = (
                        idx_pairs_upper_diag[0][i],
                        idx_pairs_upper_diag[1][i],
                    )
                    lower_score_idx = reverse_idx_tuple_pairs_lower_diag.index(
                        (upper_idx_n, upper_idx_m)
                    )
        ...
        candidate_embeddings = model.encode(candidates)
        score_matrix = (
            util.cos_sim(argument_embeddings, candidate_embeddings)
            .to("cpu")
            .detach()
            .numpy()
        )
...
def get_best_match_plus_threshold_dict(
    arguments,
    candidates,
    match_scorer,
    match_scorer_t=0.856,
    include_not_matching=False,
    return_ids=False,
    topic=None,
):
    """ """
    # Get match score matrix
    score_matrix = get_match_score_matrix(
        match_scorer, arguments, candidates, topic=topic
    )
...
def get_best_match_plus_threshold(
    match_scorer, arguments, key_points, match_scorer_t=0.856, topic=None
):
    """
    Match arguments to key points.
    """
    bm = get_best_match_plus_threshold_dict(
        match_scorer=match_scorer,
        arguments=arguments,
        candidates=key_points,
        topic=topic,
        match_scorer_t=match_scorer_t,
        include_not_matching=True,
        return_ids=True,
    )
...
def eval_match_scorer(model, df_test, include_topic, output_dir=None):
    ...
    results = {"acc": [], "f1": [], "runtime": [], "topic": [], "stance": []}
    ...
    for topic in topics:
        ...
        for stance in stances:
            ...
            kps = dict(
                zip(
                    df_kp_sta["key_point"].unique(),
                    [i for i in range(len(df_kp_sta["key_point"].unique()))],
                )
            )
            # Get references
            ...
            # Get matches
            start_time = time()
            preds = get_best_match_plus_threshold(
                match_scorer=model,
                arguments=args,
                key_points=list(kps.keys()),
                topic=topic_to_include,
                match_scorer_t=0,
            )
...
def train_bi_match_scorer_smatchtopr(df_train, include_topic=True, epochs=10):
    ...
    model = SentenceTransformer(
        modules=[word_embedding_model, pooling_model], device="mps"
    ).to("mps")
    ...
    if include_topic == True:
        train_examples = [
            InputExample(
                texts=[
                    df_train.iloc[i]["argument"],
                    df_train.iloc[i]["topic"]
                    + " </s> </s> "
                    + df_train.iloc[i]["key_point"],
                ],
        ...
        train_examples = [
            InputExample(
                texts=[df_train.iloc[i]["argument"], df_train.iloc[i]["key_point"]],
                label=df_train.iloc[i]["label"],
            )
            for i in range(len(df_train))
        ]
    ...
    evaluator = KeyPointEvaluator.from_eval_data_path(
        "data/KPA_2021_shared_task/kpm_data",
        "dev",
        name="dev",
...
def train_bi_match_scorer_barh(
    df_train,
    model_path=None,
    include_topic=True,
    epochs=10,
    ...
    if augmented == True:
        train_examples = [
            InputExample(
                texts=[df_train.iloc[i]["sentence_1"], df_train.iloc[i]["sentence_2"]],
                label=int(df_train.iloc[i]["label"]),
            )
            for i in range(len(df_train))
        ]
    else:
        if loss == "cos":
            train_examples = [
                InputExample(
                    texts=[df_train.iloc[i]["argument"], df_train.iloc[i]["key_point"]],
                    label=float(df_train.iloc[i]["label"]),
                )
                for i in range(len(df_train))
            ]
        else:
