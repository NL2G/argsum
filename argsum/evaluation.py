...
def llm_sim_eval(
    ...
    scale_min=0,
    scale_max=100,
    user_message_add_corr=True,
    user_message_few_shot=True,
    user_message_single_num_out=False,
    temperature=0.7,
):
...
"""
    if user_message_add_corr == True:
        user_message += (
            "Your generated scores should have a high correlation with human judgments."
        )
        ...
        while success == False:
            try:
                ...
                outputs.append(
                    np.mean(
                        [
                            float(re.findall(r"\b\d+(?:\.\d+)?\b", i)[0])
                            for i in [
...
def get_soft_scores(reference, candidate, similarity_functions="all"):
    ...
    if similarity_functions == "all":
        similarity_functions = [
            ...
            "all_mpnet_base_v2",
            "sbert_roberta_match",
            "s_bert_all_mpnet_base_v2_match",
            "deberta_match_1",
            "deberta_match_2",
            "deberta_match_mean",
            "llm_sim",
        ...
        zip(
            ...
            [
                dict(
                    zip(
                        ["sP", "sR", "sF1", "runtime"],
                        [np.nan for i in range(len(["sP", "sR", "sF1", "runtime"]))],
                    )
                )
    ...
    refs = list(
        itertools.chain.from_iterable(
            itertools.repeat(x, len(candidate)) for x in reference
        )
    )
    ...
    # BERTScore
    if (
        ("bert_score_p" in similarity_functions)
        or ("bert_score_r" in similarity_functions)
        or ("bert_score_f" in similarity_functions)
    ):
        start_time = time()
        bert_score_p, bert_score_r, bert_score_f = bert_score(
            cands=cands,
            refs=refs,
            lang="en",
            verbose=False,
    ...
    # BARTScore
    if (
        ("bart_score_cnndm" in similarity_functions)
        or ("bart_score_cnndm_rs" in similarity_functions)
        or ("bart_score_parabank" in similarity_functions)
        or ("bart_score_parabank_rs" in similarity_functions)
    ):

        if ("bart_score_cnndm" in similarity_functions) or (
            "bart_score_cnndm_rs" in similarity_functions
        ):
            bart_scorer_cnndm = BARTScorer(
            ...
            if "bart_score_cnndm_rs" in similarity_functions:
                bart_score_cnndm_rs = np.array(
                    [rescale_bart(x) for x in bart_score_cnndm]
                )
                results["bart_score_cnndm_rs"]["runtime"] = bart_score_cnndm_runtime
                all_scores.append(bart_score_cnndm_rs)

        elif ("bart_score_parabank" in similarity_functions) or (
            "bart_score_parabank_rs" in similarity_functions
        ):
            ...
            start_time = time()
            bart_score_parabank = np.array(
                bart_scorer_parabank.score(cands=cands, refs=refs)
            )
            ...
            if "bart_score_parabank_rs" in similarity_functions:
                bart_score_parabank_rs = np.array(
                    [rescale_bart(x) for x in bart_score_parabank]
                )
                results["bart_score_parabank_rs"][
                    "runtime"
                ] = bart_score_parabank_runtime
    ...
    if "all_mpnet_base_v2" in similarity_functions:
        ...
        start_time = time()
        embeddings_cands = all_mpnet_base_v2_scorer.encode(
        ...
        all_mpnet_base_v2 = (
            util.cos_sim(embeddings_cands, embeddings_refs).diag().detach().numpy()
        )
    ...
    if "sbert_roberta_match" in similarity_functions:
        sbert_roberta_match_scorer = SentenceTransformer(
            "models/match_scorer/bi_encoder/roberta_tp/2024-Feb-20_16-23-49"
        )
        start_time = time()
        embeddings_cands = sbert_roberta_match_scorer.encode(
            refs, convert_to_tensor=True
        )  # Match Score: roberta_tp
        embeddings_refs = sbert_roberta_match_scorer.encode(
            cands, convert_to_tensor=True
        )
        sbert_roberta_match = (
            util.cos_sim(embeddings_cands, embeddings_refs).diag().detach().numpy()
        )
    ...
    if "s_bert_all_mpnet_base_v2_match" in similarity_functions:
        s_bert_all_mpnet_base_v2_match_scorer = SentenceTransformer(
            "models/match_scorer/bi_encoder/all-mpnet-base-v2/2024-Jun-05_22-29-27/checkpoints/6200"
        )
        start_time = time()
        embeddings_cands = s_bert_all_mpnet_base_v2_match_scorer.encode(
            refs, convert_to_tensor=True
        )  # Match Score: all-mpnet-base-v2
        embeddings_refs = s_bert_all_mpnet_base_v2_match_scorer.encode(
            cands, convert_to_tensor=True
        )
        s_bert_all_mpnet_base_v2_match = (
            util.cos_sim(embeddings_cands, embeddings_refs).diag().detach().numpy()
    ...
    # DeBERTa Match Score
    if (
        ("deberta_match_1" in similarity_functions)
        or ("deberta_match_2" in similarity_functions)
        or ("deberta_match_mean" in similarity_functions)
    ):
        deberta_match_scorer = torch.load(
            "models/match_scorer/cross_encoder/deberta-v2_np/2024-Feb-20_13-41-02/best_model.pt"
        )

        if "deberta_match_1" in similarity_functions:
            start_time = time()
            deberta_match_1 = get_match_scores(
                arguments=refs, candidates=cands, model=deberta_match_scorer
            )
        ...
        if "deberta_match_2" in similarity_functions:
            start_time = time()
            deberta_match_2 = get_match_scores(
                arguments=cands, candidates=refs, model=deberta_match_scorer
            )
        ...
        if "deberta_match_mean" in similarity_functions:
            ...
            results["deberta_match_mean"]["runtime"] = (
                deberta_match_1_runtime + deberta_match_2_runtime
            )
...
def extract_coverage_count(response_text):
    """
    Extracts the coverage count from the response text.
    """
    match = re.search(r"Coverage count:\s*(\d+(?:\.\d+)?)", response_text)
    if match:
        return float(match.group(1))
    else:
        print("No coverage count found in response text.")
        return None


def match_scorer_cov_eval(
    reference, candidate, match_scorer=None, threshold=0.85, roberta_tokenizer=None
):
...
""".format(
            topic=topic,
            stance_=stance_dict[stance],
            reference_str=reference_str,
            candidate_str=candidate_str,
            max=len(reference_str.splitlines()),
        )
...
""".format(
    ...
    while (success == False) and (n_failed < 3):
        try:
            output = client.chat.completions.create(
                model="gpt-4o-mini", messages=input, temperature=temperature, n=10
            )
            filtered_output = [
                extract_coverage_count(content)
                for content in [
                    output.choices[i].message.content
                    for i in range(len(output.choices))
                ]
            ...
            filtered_output = [x for x in filtered_output if x is not None]
...
def extract_uniqueness_count(response_text):
    """
    Extracts the uniqueness count from the response text.
    """
    match = re.search(
        r"Number of Unique Main Statements:\s*(\d+(?:\.\d+)?)", response_text
    )
    if match:
        return float(match.group(1))
    else:
        print("No uniqueness count found in response text.")
        return None
...
Concisely provide your process as outlined in the explanation above. 
...
""".format(
    ...
    while (success == False) and (n_failed < 3):
        try:
            ...
            filtered_output = [
                extract_uniqueness_count(content)
                for content in [
                    output.choices[i].message.content
                    for i in range(len(output.choices))
                ]
            ...
            if len(filtered_output) == 0:
                ...
                for i in range(len(output.choices)):
                    print(f"Output {i}:")
                    print(output.choices[i].message.content)
                n_failed += 1
                continue
